...
RtAudioOutput::RtAudioOutput(uint32_t channelCount,
                             uint32_t sampleSize,
                             uint32_t sampleRate)
  ...
  dac_ = std::find(apis.begin(), apis.end(), RtAudio::LINUX_PULSE) == apis.end()
             ? std::make_unique<RtAudio>()
...
int RtAudioOutput::audioBufferReadHandler(void* outputBuffer,
                                          void* inputBuffer,
