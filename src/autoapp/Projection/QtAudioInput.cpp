...
QtAudioInput::QtAudioInput(uint32_t channelCount,
                           ...
                           uint32_t sampleRate)
    : ioDevice_(nullptr) {
  qRegisterMetaType<IAudioInput::StartPromise::Pointer>(
      "StartPromise::Pointer");
  ...
  connect(this, &QtAudioInput::startRecording, this,
          &QtAudioInput::onStartRecording, Qt::QueuedConnection);
  connect(this, &QtAudioInput::stopRecording, this,
          &QtAudioInput::onStopRecording, Qt::QueuedConnection);
  QMetaObject::invokeMethod(this, "createAudioInput",
...
void QtAudioInput::onReadyRead() {
  ...
  auto readSize =
      ioDevice_->read(reinterpret_cast<char*>(buffer.data), buffer.size);
