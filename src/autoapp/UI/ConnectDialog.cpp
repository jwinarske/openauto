...
ConnectDialog::ConnectDialog(
  ...
  connect(ui_->listViewRecent, &QListView::clicked, this,
          &ConnectDialog::onRecentAddressClicked);
  connect(this, &ConnectDialog::connectionSucceed, this,
          &ConnectDialog::onConnectionSucceed);
  connect(this, &ConnectDialog::connectionFailed, this,
          &ConnectDialog::onConnectionFailed);
...
void ConnectDialog::onConnectionSucceed(
    const aasdk::tcp::ITCPEndpoint::SocketPointer&,
    const std::string& ipAddress) {
...
void ConnectDialog::onConnectionFailed(const QString& message) {
  ...
  QMessageBox errorMessage(QMessageBox::Critical, "Connect error", message,
                           QMessageBox::Ok);
...
void ConnectDialog::onRecentAddressClicked(const QModelIndex& index) {
  ...
  if (static_cast<size_t>(index.row()) <= recentAddressesList.size()) {
    ui_->lineEditIPAddress->setText(
        QString::fromStdString(recentAddressesList.at(index.row())));
  }
}
