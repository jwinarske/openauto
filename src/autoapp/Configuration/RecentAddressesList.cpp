...
constexpr char RecentAddressesList::cRecentEntriesCount[];
...
void RecentAddressesList::load() {
  ...
  try {
    ...
    const auto listSize =
        std::min(maxListSize_, iniConfig.get<size_t>(cRecentEntriesCount, 0));

    for (size_t i = 0; i < listSize; ++i) {
      const auto key = cRecentEntryPrefix + std::to_string(i);
      const auto address = iniConfig.get<RecentAddresses::value_type>(
          key, RecentAddresses::value_type());
    ...
    OPENAUTO_LOG(warning)
        << "[RecentAddressesList] failed to read configuration file: "
...
void RecentAddressesList::save() {
  ...
  const auto entriesCount = std::min(maxListSize_, list_.size());
  iniConfig.put<size_t>(cRecentEntriesCount, entriesCount);
