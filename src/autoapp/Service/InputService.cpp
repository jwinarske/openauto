...
void InputService::onButtonEvent(const projection::ButtonEvent& event) {
  ...
  asio::dispatch(strand_, [this, self = this->shared_from_this(),
    ...
    if (event.code == aasdk::proto::enums::ButtonCode::SCROLL_WHEEL) {
      ...
      relativeEvent->set_delta(
          event.wheelDirection == projection::WheelDirection::LEFT ? -1 : 1);
      relativeEvent->set_scan_code(event.code);
    } else {
      auto buttonEvent =
          inputEventIndication.mutable_button_event()->add_button_events();
      buttonEvent->set_meta(0);
      buttonEvent->set_is_pressed(event.type ==
                                  projection::ButtonEventType::PRESS);
