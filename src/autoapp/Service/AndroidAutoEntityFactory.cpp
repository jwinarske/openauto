...
IAndroidAutoEntity::Pointer AndroidAutoEntityFactory::create(
  ...
  auto transport(std::make_shared<aasdk::transport::USBTransport>(
      ioService_, std::move(aoapDevice)));
...
IAndroidAutoEntity::Pointer AndroidAutoEntityFactory::create(
  ...
  auto transport(std::make_shared<aasdk::transport::TCPTransport>(
      ioService_, std::move(tcpEndpoint)));
  return create(std::move(transport));
}
...
IAndroidAutoEntity::Pointer AndroidAutoEntityFactory::create(
  ...
  auto cryptor(
      std::make_shared<aasdk::messenger::Cryptor>(std::move(sslWrapper)));
  cryptor->init();

  auto messenger(std::make_shared<aasdk::messenger::Messenger>(
      ioService_,
      std::make_shared<aasdk::messenger::MessageInStream>(ioService_, transport,
                                                          cryptor),
  ...
  return std::make_shared<AndroidAutoEntity>(
      ioService_, std::move(cryptor), std::move(transport),
