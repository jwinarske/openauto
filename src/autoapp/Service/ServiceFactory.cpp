...
IService::Pointer ServiceFactory::createVideoService(
  ...
  projection::IVideoOutput::Pointer videoOutput(
      ...
      std::bind(&QObject::deleteLater, std::placeholders::_1));
#endif
  return std::make_shared<VideoService>(ioService_, messenger,
...
IService::Pointer ServiceFactory::createBluetoothService(
  ...
  return std::make_shared<BluetoothService>(ioService_, messenger,
...
void ServiceFactory::createAudioServices(
  ...
  if (configuration_->musicAudioChannelEnabled()) {
    ...
    serviceList.emplace_back(std::make_shared<MediaAudioService>(
  ...
  if (configuration_->speechAudioChannelEnabled()) {
    ...
    serviceList.emplace_back(std::make_shared<SpeechAudioService>(
        ioService_, messenger, std::move(speechAudioOutput)));
  }
  ...
  serviceList.emplace_back(std::make_shared<SystemAudioService>(
